\begin{frame}\frametitle{Why look at centrifuges?}
	\begin{itemize}
...
\begin{frame}\frametitle{Laboratory centrifuges}
	...
	\begin{columns}[t]
		...
		\column{0.60\textwidth}
			\begin{itemize}
				\item	$S$ = Svedberg coefficient of the material (from tables, experiments) 
...
\begin{frame}\frametitle{Theoretical trajectories: tubular bowl centrifuge}
	\begin{itemize}
		...
		\item	The particle is also forced in the direction of the fluid, so net trajectory:
		\begin{center}
...
\begin{frame}\frametitle{Theoretical trajectories: tubular bowl centrifuge}
	...
	\]
	\begin{itemize}
		\item	Alternative interpretation: for a given flow $Q$, find the largest particle diameter that will arrive exactly at $r_2$
		\item	Assume particles with smaller $D_p$ leave in supernatant
		\item	Obviously this is excessive: we have a discharge weir to \emph{retain particles} that might not have reached $r_2$
	\end{itemize}	
\end{frame}

\begin{frame}\frametitle{Cut-size diameter}
	So to prevent excessive over design, we rather find a mid point between $r_1$ and $r_2$, and solve the equations for the diameter of the particle that reaches this \emph{cut point}:
	\[
...
\begin{frame}\frametitle{Sigma theory for centrifuges}
	...
	\]
	\[
		\Sigma \approx \frac{\omega^2}{g} \pi h \left(1.5r_2^2 + 0.5r_1^2 \right)
	\]
...
\begin{frame}\frametitle{Why use the Sigma term?}
	\begin{itemize}
		...
		\item	Centrifuge 1: $Q_{\text{cut},1} = 2 v_\text{TSV} \cdot \Sigma_1$
		\item	Centrifuge 2: $Q_{\text{cut},2} = 2 v_\text{TSV} \cdot \Sigma_2$
		\item	\[
					\frac{Q_{\text{cut},1}}{Q_{\text{cut},2}}  = \frac{\Sigma_1}{\Sigma_2}
				\]
...
\begin{frame}\frametitle{More on the tubular bowl centrifuge}
	Can be used for fluid-fluid separation
	\begin{columns}[c]
		\column{0.60\textwidth}
			\begin{center}
...
\begin{frame}\frametitle{Disk-bowl centrifuge}
	\begin{itemize}		
		...
		\item	Adding angled disks gives a greater surface area, hence volume, without increasing diameter $r_2$
		...
		\item	Periodic cleaning is required though
