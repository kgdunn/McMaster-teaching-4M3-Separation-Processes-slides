...
\begin{frame}\frametitle{Tubular bowl centrifuge}
	...
	\end{center}
	\vspace{-24pt}	
	\see{Geankoplis, Fig 14.4-2}; {\tiny \emph{draw top view}}
\end{frame}
...
\begin{frame}\frametitle{Example}
	...
	\begin{itemize}
		...
		\item	800 revolutions per \st{minute} {\color{red}second}
	\end{itemize}
...
\begin{frame}\frametitle{Sigma theory for centrifuges}
	Take the previous equation for $Q_\text{cut} = \dfrac{V}{t_\text{cut}}$, multiply numerator and denominator by $g$, then substitute Stokes' law for particles settling under gravity:
	\[
	...
	\begin{columns}[c]
		\column{0.60\textwidth}
			\begin{center}
				...
				\vspace{-.5cm}
				\qquad\see{Geankoplis, Fig 14.4-33}
			\end{center}
	...
	\begin{columns}[c]
		\column{0.50\textwidth}
			...
			\begin{itemize}
				...
				\item	$r_1$ = outer cone radius (\emph{different!})
...
\begin{frame}\frametitle{Sequencing of centrifuges: to get an overall separation of multiple streams}
	\begin{center}
...
\begin{frame}\frametitle{Details around the secondary unit in the prior flowsheet}
	\centerline{\includegraphics[width=.8\textwidth]{\imagedir/separations/centrifuge/westfalia-separator--ODB260_EN.jpg}}
...
\begin{frame}\frametitle{Safety}
	\begin{itemize}
		...
		\end{itemize}
		\item	flammable fluids, such as solvents must be blanketed in an environment where ignition can occur: use nitrogen, or any other inert gas, as a ``blanket''
	\end{itemize}
\end{frame}
...
\begin{frame}\frametitle{Design a centrifuge for beer clarification}
	...
	\vspace{12pt}
	The beer is produced in batches of 100 $\text{m}^3$, with 4 batches per day.
	...
	\vspace{12pt}
	\onslide+<2->{
	See the spreadsheet at \href{http://yint.org/centrifuge}{http://yint.org/centrifuge}}
\end{frame}
...
\begin{frame}\frametitle{Further practice questions}
	\begin{enumerate}
		...
		\vspace{12pt}
		\item	Continuing from above: what will be the corresponding size cut for a suspension of coal particles in oil fed at the rate of $0.04~\text{m}^{3}\text{.s}^{-1}$? The density of coal is $1300~\text{kg.m}^{-3}$ and the density of the oil is $850~\text{kg.m}^{-3}$ and its viscosity is $0.01~\text{N.s.m}^{-2}$. {\color{myOrange}\tiny{[\emph{Ans}: $D_\text{p,cut} = 4$\micron]}}
