...
\begin{frame}\frametitle{Administrative}
	...
	\begin{itemize}
		...
		\item	Update to question 3.3.3 (or question 3.3(c))
			...
			\end{itemize}
		\item	Assignment 4 available for pick up
	\end{itemize}
	\vspace{12pt}
	...
	\end{exampleblock}
	\begin{itemize}
		\item	Currently 39\% of the class have filled it in
	\end{itemize}
	\begin{center}
		\includegraphics[width=0.30\textwidth]{\imagedir/separations/admin/course-evals-qr-code.png}
	\end{center}
\end{frame}
...
\begin{frame}\frametitle{Exercise}
	...
	\vspace{-12pt}
	\hfill\includegraphics[width=0.70\textwidth]{\imagedir/separations/drying/adiabatic-saturation-temperature.png}
...
\begin{frame}\frametitle{Some heat-transfer correlations}
	...
	\begin{enumerate}
		...
		\begin{itemize}
			...
			\vspace{5pt}
			\item	$\mathbf{h = 0.0204G^{0.8}}\,\,[\text{W.m}^{-2}\text{.K}^{-1}]$ \hfill {\color{myRed}{$\longleftarrow$ awkward units for $G$!}}
		\end{itemize}
		\vspace{12pt}
	...
	\end{enumerate}
	\vspace{12pt}
	See textbooks for $h$ when using pelletized solids (e.g packed bed)
\end{frame}
...
\iftoggle{instructor}{
	\begin{frame}\frametitle{Why these equations makes sense}
		...
		\end{center}
		\vspace{-6pt}
		\[\frac{(\Delta M_\text{water}) (\Delta H_\text{vap})}{hA(T_\text{db} - T_\text{wb})} = \text{time to remove $\Delta M_\text{water}$}
		\]
		\[h = a(G)^{b} = a(\rho v)^{b}\]
	\end{frame}
}{}
...
\begin{frame}\frametitle{Shelf/tray dryer}
...
\begin{frame}\frametitle{Filter cake drying example}
	\begin{enumerate}
		...
		\begin{itemize}
			\item	$G = 3600\, \rho v_\text{avg} = 3600 (1.048)^{-1} \times 4 =$  {\color{myOrange}{$13\,740\,\text{kg.hr}^{-1}\text{.m}^{-2}$}}
			\item	$h = 0.0204 (13740)^{0.8} =$ {\color{myOrange}{$h = 41.7\,\,\text{W.m}^{-2}\text{.K}^{-1}$}}
		\end{itemize}
...
\begin{frame}\frametitle{Some equipment examples}
	...
	\begin{columns}[t]
		...
		\column{0.65\textwidth}
			\begin{itemize}
				\item	5 to 50 kg/hr/m$^3$ dryer volume evaporated
