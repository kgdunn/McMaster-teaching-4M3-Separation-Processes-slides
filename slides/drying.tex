...
\begin{frame}\frametitle{The nature of water in solid material}
	...
	\begin{columns}[c]
		...
		\column{0.50\textwidth}
			...
			% 2015: be careful here: bound and unbound are terms used at 100% humidity. At other levels use the free and equilibrium amounts.
	\end{columns}
\end{frame}
...
\begin{frame}\frametitle{Exercise}
	...
	{\small
	\begin{enumerate}
		\item	What is the percentage humidity of the incoming air stream? \iftoggle{instructor}{\visible<2->{{\color{myOrange}{[20\%]}}}}{}
		\item	What is the percentage humidity of the air stream leaving?  \iftoggle{instructor}{\visible<2->{{\color{myOrange}{[100\%]}}}}{}
		\item	What is the humidity [mass/mass] of the air stream leaving? \iftoggle{instructor}{\visible<2->{{\scriptsize{\color{myOrange}{[66g/kg]}}}}}{}
		\item	What is the temperature of the air stream leaving? \iftoggle{instructor}{\visible<2->{{\color{myOrange}{[45\degC]}}}}{}
		...
		\iftoggle{instructor}{\visible<2->{{\color{myOrange}{[$(66-55) = 11$ g per min]}}}}{}
	\end{enumerate}}
...
\begin{frame}\frametitle{Example}
	...
	\vspace{12pt}
	\begin{itemize}
		\item	the recorded dry-bulb temperature \iftoggle{instructor}{\visible<2->{ {\color{myOrange}[55$^\circ$C]}}}{}
		\item	percentage humidity  \iftoggle{instructor}{\visible<2->{{\color{myOrange}[26\%]}}}{}
		\item	dew point temperature  \iftoggle{instructor}{\visible<2->{{\color{myOrange}[$\approx 31^\circ$C]}}}{}
		\item	humid heat   \iftoggle{instructor}{\visible<2->{{\color{myOrange}$\left[c_S = 1.061 \dfrac{\text{kJ}}{\text{(kg dry air)(K)}}\right]$}}}{}
		\item	humid volume  \iftoggle{instructor}{\visible<2->{{\color{myOrange}[$T=328\text{K}$; $v_H = 0.973 \text{m}^3/(\text{kg moist air})$ ]}}}{}
		\item	wet-bulb temperature \iftoggle{instructor}{\visible<2->{ {\color{myOrange}[$\approx 36^\circ$C]}}}{}
	\end{itemize}
...
\begin{frame}\frametitle{How to choose the equipment*}
	\begin{itemize}
		...
		\end{itemize}
		\item	Heating choice direction:
		\begin{itemize}
			\item	temperature-sensitive products dried co-current
			\item	counter-current more energy efficient
		\end{itemize} 
		\item	Agitation:
		\begin{itemize}
	...
	\begin{columns}[t]
		\column{0.45\textwidth}
			\begin{itemize}
				...
				\item	use to dry: sand, ore, resins, fibres, pulp
			\end{itemize}
...
\begin{frame}\frametitle{Some heat-transfer correlations for $h$}
	...
	\vspace{6pt}
	In all cases here: $G = 3\,600\,\rho v_\text{avg}$ where $v$ and $\rho$ are in SI units and $G$ is in \([\text{kg.hr}^{-1}\text{.m}^{-2}]\) already
	\begin{enumerate}
		\item	Parallel flow to surface:
		\begin{itemize}
			\item	Air has $T_\text{db}$ between 45 to 150\degC
...
% 2015: fix up this slide to reveal less of the answer. Students should plan it themselves
% 2015: split the slide into two parts: Q1 to 4 and then Q5 on a separate slide
\begin{frame}\frametitle{Filter cake drying example}
	\begin{enumerate}
		\item	What is the humidity of the incoming air stream? \visible<2->{\\{\scriptsize \color{myOrange}{[$\psi$ = \iftoggle{instructor}{0.04}{\_\_\_\_\_\_} kg water/kg dry air]}}}
		\item	What is the wet-bulb temperature of this air stream? \visible<2->{{\scriptsize \color{myOrange}{[$T_\text{wb} \approx \iftoggle{instructor}{41.3}{\_\_\_\_\_\_}$\degC]}}}
		\item	What is the humid volume of the drying air stream? \visible<2->{{\scriptsize \color{myOrange}{[$T_\text{db}=\iftoggle{instructor}{348}{\_\_\_\_\_\_}$K, $v_H = \iftoggle{instructor}{1.048}{\_\_\_\_\_\_} \text{m}^3\text{.kg}^{-1}$]}}}
		\item	Estimate the heat transfer coefficient.
		\begin{itemize}
			\item	$G = 3\,600\, \rho v_\text{avg} = 3\,600 (1.048)^{-1} \times 4 =$  \visible<2->{{\color{myOrange}{$\iftoggle{instructor}{13\,740}{\_\_\_\_\_\_}\,\text{kg.hr}^{-1}\text{.m}^{-2}$}}}
			\item	$h = 0.0204 (13\,740)^{0.8} =$ \visible<2->{{\color{myOrange}{$h = \iftoggle{instructor}{41.7}{\_\_\_\_\_\_}\,\,\text{W.m}^{-2}\text{.K}^{-1}$}}}
		\end{itemize}
		\item	Substitute into the constant-drying rate expression to find:
		\begin{itemize}
			\item	drying time = $\dfrac{(\Delta M_\text{water}) (\Delta H_\text{vap})}{hA(T_\text{db} - T_\text{wb})} =$ {\scriptsize $\displaystyle \frac{(19.5)(2401\times 1000)}{(41.7)(2)(75-41.3)} =$} \visible<2->{{\\ \color{myOrange}{drying time = $\mathbf{\iftoggle{instructor}{4.6}{\_\_\_\_\_\_}\,\textbf{hrs}}$}}}
			...
			\item	We need the $\Delta H_\text{vap}$ at $T_\text{wb}$ (why?) \visible<2->{{\small \color{myOrange}{[\iftoggle{instructor}{2401}{\_\_\_\_\_\_} kJ.kg$^{-1}$.K$^{-1}$]}}}
			\begin{itemize}
				\item	2501 kJ/kg at 0\degC
				\item	2260 kJ/kg at 100\degC 
			\end{itemize}
...
\begin{frame}\frametitle{Example: extended}
	...
	\vspace{12pt}
	{\color{myGreen}What about spherical pellets of particles and using a packed bed?}
	\begin{itemize}
