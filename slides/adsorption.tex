...
% For 2015: update the market segment values for adsorbents. Your data is from 1983
...
\begin{frame}\frametitle{This section in context of the course}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item	filtration (e.g. vacuum filtration)
	...
	\begin{columns}[t]
		\column{0.75\textwidth}
			...
			\begin{itemize}
				...
				\item	ion-exchange: passing water through certain sand deposits removes salt % see "2013-4M3-Conant-Ward-Project" for some additional background
	...
	\begin{columns}[c]
		...
		\column{0.40\textwidth}
			...
			\begin{itemize}
				...
				\item	one of the most widely used adsorbents
				\item	hydrophilic
	...
	\begin{columns}[c]
		...
		\column{0.50\textwidth}
			...
			\begin{itemize}
				...
				\item	hydrophobic
				\item	low bonding strength (good for regeneration)
...
\begin{frame}\frametitle{H$_2$S and CO$_2$ pre-treatment adsorbers}
	...
	\end{center}
	\see{\href{http://www.flickr.com/photos/vmeprocess/4565083761}{Flickr: vmeprocess}}
...
\begin{frame}\frametitle{When to consider adsorption}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item	{\color{myOrange}{we often have to pump them, filter them, and/or put them through cyclones to process the solid phase}}
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}\frametitle{Physical principles}
	\begin{exampleblock}{Adsorption releases heat, it's exothermic. Why?}
		...
		% Specifically, Delta G is negative for a spontaneous reaction to occur
	\end{exampleblock}
...
\begin{frame}\frametitle{Packed beds: adsorption and desorption steps}
	...
	\begin{enumerate}
		\item	\textbf{raise the temperature} to shift the equilibrium (isotherm)
...
\begin{frame}\frametitle{(Fluidized bed?) example}
	...
	\emph{\small Zoom in on the high resolution photo to see details.}
\end{frame}
...
\begin{frame}\frametitle{Understanding adsorption in packed beds (1 of 2)}
	...
	\end{center}
	\vspace{-12pt}
	{\footnotesize $L$ = bed length; $\theta$ = time; $\theta_0$ = time at which a regenerated bed is started up}
\end{frame}
...
\begin{frame}\frametitle{Understanding adsorption in packed beds (2 of 2)}
	...
	\end{center}
	\vspace{-24pt}
	\see{Prior slide and this slide from: Lukchis}
	\vspace{-18pt}
	\begin{columns}[t]
		\column{1.1\textwidth}
			\begin{itemize}
...
\end{frame}
\end{comment}
% For 2014: the problem is "usable capacity in an ideal wavefront = 3.65/6.9 = 53%" <---- don't put that question in.
% In the example, cover that LES / L = (A1+A2) / AREA, where AREA = total (x,y) area = (6.9 - 0)*(1-0) = 6.9 hours
...
\begin{frame}\frametitle{Bed mass balance}
	...
	\begin{tabular}{llr}\vspace{0pt}
		$Q_F $					& Feed flow rate					& $\left[ \dfrac{\text{m}^3\text{fluid}}{\text{second}}\right]$ \\ 	\vspace{2pt}
		...
		$C_\text{A,S}^{e}$		& Equilibirum adsorbed solute conc$^\text{n}$& $\left[ \dfrac{\text{kg solute}}{\text{kg adsorbent}}\right]$ \\ \vspace{2pt}
...
\begin{frame}\frametitle{Another question from a previous exam}

	The isotherm for benzene, at 25$^\circ C$, on an activated carbon adsorbent is given as: $C_\text{A,S} = 32 C_\text{A}^{0.428}$
	...
	You want to create your own adsorber packed bed from a piece of piping that has diameter of 24.5 cm. The activated carbon supplier has given you the following specification sheet:	
	
	\begin{itemize}
		\item	activated carbon bulk density = 410 g/L
		\item	activated carbon particle density = 520 g/L
	\end{itemize}

	You want a breakthrough time of 4 hours for a feed stream of 2.8 g of benzene per litre. You must treat 30 L/min of waste water.
	 
	\begin{enumerate}
		\item	How long should your packed bed be? Be clear with any simplifying assumptions you make. Use the approximation that if you don't know MTZ, that your MTZ = 4ft [1.22m], and assuming a symmetric wavefront, that the $\text{LUB} = 2 \times \text{MTZ}$.
		\item	What will be the cost of the adsorbent you need to purchase [\$5.50 per kilogram activated carbon]? 
	\end{enumerate}
\end{frame}
...
\begin{frame}\frametitle{Adsorption equipment: Sorbex column}
	...
	\begin{center}
		\includegraphics[width=\textwidth]{\imagedir/separations/adsorption/Sorbex-Seader-p611.png}
	\end{center}
	\vspace{-12pt}
	\see{Seader, Henley, Roper, p 611}

	{\tiny A) Pump; B) Adsorbent chamber; C) \href{http://www.uop.com/equipment/aromatics-separation/}{Rotary valve}; D) Extract column; E) Raffinate column}
	
	% As described by Broughton [139], the bed is held sta- tionary in one column, which is equipped with a number (perhaps 12) of liquid feed entry and discharge locations. By shifting, with a rotary valve (RV), the locations of feed entry, desorbent entry, extract (adsorbed) removal, and raffinate (nonadsorbed) removal, countercurrent movement of solids is simulated by a downward movement of liquid. For the valve positions shown in Figure 15.43, Lines 2 (entering desorbent), 5 (exiting extract), 9 (entering feed), and 12 (exit- ing raffinate) are operational, with all other numbered lines closed. Liquid is circulated down through and, externally, back up to the top of the column by a pump. Ideally, an infi- nite number of entry and exit locations exist and the valve would continuously change the four operational locations. Since this is impractical, a finite number of locations are
