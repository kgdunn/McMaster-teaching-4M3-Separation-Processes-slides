...
\begin{frame}\frametitle{Regenerating the bed}
	...
	\begin{itemize}
		...
		\item	often leave residual solvent, since time to completely strip adsorbent would be excessive
		\item	care must be taken with flammable adsorbates:
		\begin{itemize}
			\item	stripping temperature are high
...
\begin{frame}\frametitle{Example}
	...
	\begin{columns}[t]
		...
		\begin{enumerate}
			\item	Determine the {\color{purple}{breakpoint time}}, $\theta_b$ \iftoggle{instructor}{[{\color{myOrange}{Ans:}} 3.65 hours]}{}		
			\pause
			\item	What is the {\color{purple}{usable capacity}} of the bed? \iftoggle{instructor}{[{\color{myOrange}{Ans:}} the area $A_1$, up to time $\theta_b = 3.65~\text{hours}$]}{}
			\pause
				\begin{itemize}
			...
			\item	How long does it take to reach this capacity? 3.65 \text{hours}, but if we want to be exact, it is $t_\text{used} = \displaystyle \int_0^{\theta_B}{\left(1-\frac{C_\text{A}}{C_\text{A,F}} \right)dt} \approx 3.65$
		\end{enumerate}
	\end{columns}	
\end{frame}
	...
	\begin{columns}[t]
		...
		\begin{enumerate}
			...
			\item	But what actual fraction of the bed's capacity is used at $\theta_b$?		
				\begin{center}
					\includegraphics[width=0.7\textwidth]{\imagedir/separations/adsorption/various-times-from-bed.png}
				\end{center}
				\vspace{-12pt}
				\begin{itemize}
					...
					\item	{\color{purple}{stoichiometric time}}, $\theta_S$, is defined as \textbf{time taken} for this actual capacity to be used
	...
	\begin{columns}[t]
		...
		\begin{enumerate}
			...
			\item	If the lab-scale bed were originally 14cm long, what length is unused at time $\theta_b$?
				\begin{itemize}
					\item	intuitively: $14 - 0.71 \times 14 = 4.1$ cm
					\item	$L_\text{UNB} = \left(1 - \displaystyle\frac{t_\text{used}}{\theta_S} \right)L_\text{total} = \left(1 - \displaystyle\frac{3.65}{5.15} \right)14 = 4.1$ cm
					\item	where $L_\text{UNB}$ is the {\color{purple}{length of the unused bed}}
			...
			\begin{itemize}
				\item	Ratio the bed lengths: $\displaystyle \frac{7.5}{3.65} \times 0.71(14) = 20.4$ cm
				\item	But that's the length to get breakthrough in 7.5 hours
				\item	We have to add on the length of the unused bed = 4.1 cm from before (same diameter!)
				\item	So new bed length = 20.4 + 4.1  = 24.5 cm
