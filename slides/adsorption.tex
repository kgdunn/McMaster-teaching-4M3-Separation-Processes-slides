...
\begin{frame}\frametitle{This section in context of the course}
	...
	\begin{itemize}
		...
		\item	preliminary sizing of a adsorption unit
	\end{itemize}
\end{frame}
...
\begin{frame}\frametitle{Sorption examples}
	...
	Industrial use of adsorption picked up with synthetic manufacturing of molecular zeolites in the 1960s.
\end{frame}
...
\begin{frame}\frametitle{Adsorbents}
	...
	{Electrostatic fields exist inside the zeolite cavity: strong interactions with polar molecules. Not only based on shape/size exclusion.}
	...
	\begin{columns}[c]
		\column{1.08\textwidth}
			\begin{itemize}
				...
				\item	Leaching: $4\text{Au}\text{\scriptsize (s)} + 8\text{NaCN} + \text{O}_2 + 2 \text{H}_2\text{O} \rightleftharpoons  4 \text{Na[Au(CN)}_2\text{]} + 4 \text{NaOH}$
				\item	Adsorption: aurocyanide complex, $\text{Au(CN)}_2^{-}$, is adsorbed onto activated carbon
					\begin{itemize}
						...
						\item	separates the solid gold, Au{\scriptsize (s)}, from the pulp (slurry)
...
\begin{frame}\frametitle{When to consider adsorption}
	...
	\begin{itemize}
		...
		\item	MSA will break down mechanically over time as my move it around
		\begin{itemize}
