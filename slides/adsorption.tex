...
\begin{frame}\frametitle{Example}
	...
	\begin{columns}[t]
		...
		\begin{enumerate}
			\item	Determine the {\color{red}{\sout{breakpoint}}} {\color{purple}{breakthrough time}}, $\theta_b$. \pause \iftoggle{instructor}{[{\color{myOrange}{\small Ans: 3.65 hours}}]}{}		
...
\begin{frame}\frametitle{Bed mass balance}
	...
	\begin{exampleblock}{Amount of material loaded into the bed up to $\theta_b$}
		\[Q_F C_\text{A,F} \theta_b = C_\text{A,S} \rho_\text{S}  \left(1-\epsilon_b \right) A L_b \]
	\end{exampleblock}
	...
	\begin{tabular}{llr}\vspace{6pt}
		...
		$C_\text{A,S}$			& Adsorbed solute conc		& $\left[ \displaystyle\frac{\text{kg solute}}{\text{kg adsorbent}}\right]$ \\ \vspace{6pt}		
		$\rho_\text{S}$			& Adsorbent density			& $\left[ \displaystyle\frac{\text{kg adsorbent charged}}{\text{m}^3~\text{of packed bed}}\right]$ \\ 	\vspace{6pt}	
		$\epsilon_b$			& Bed voidage				& $\left[ - \right]$ \\  		
		$A L_b $				& Bed volume = area $\times$ length	& $\left[ \text{m}^3\right]$
	\end{tabular}
	\vspace{18pt}
	{\color{myBlue}{Equate these to determine mass of adsorbent required.}}
\end{frame}
...
\begin{frame}\frametitle{Modified from a previous exam}
	...
	\begin{enumerate}
		\item	How much TME is adsorbed when the breakthrough concentration reaches 1 ppm?
		\item	Assuming the tank is large, allowing a long residence time, give a rough estimate of how long it will take to reach this detectable outlet concentration?
	\end{enumerate}
\end{frame}
