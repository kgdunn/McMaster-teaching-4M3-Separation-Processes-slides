...
% 	\pause
% 	\begin{itemize}
% 		\item	$J = \displaystyle \rho_f	 \frac{Q_p}{A} = \displaystyle \frac{\Delta P}{\mu \left(R_m \ell_m + R_c L_c\right)} = \frac{\Delta P}{\mu \left(R'_m + R'_c\right)}$
% 
% 		\vspace{6pt}
% 		\item	$R'_m$: estimated using pure solvent through membrane at $\Delta P$
% 
% 		\vspace{6pt}
% 		\item	$R'_c$: obtained from plots of $V$ against $t$ at a fixed $\Delta P_i$
% 			\begin{itemize}
% 				\item	as we saw in {\color{myOrange}Filtration} section we can estimate $R_c$
% 				\item	vary 
% 				find $J_i$ (interpolate) that gives required $Q_p$ by varying $A$
% 			\end{itemize}
% 	\end{itemize}
% \end{frame}
...
\begin{frame}\frametitle{A \textbf{very crude} estimate of the membrane resistance, $R'_m$}
	\begin{itemize}
		...
		\item	The velocity in this tube is \[v = \dfrac{(D^2)(\Delta P)}{32 \mu \ell_M} \qquad \qquad \text{Hagen-Poiseulle}\]
		\item	Consider a $1~\text{m}^2$ surface of tube pore openings, where $\epsilon$ is the fraction of pore openings of diameter $D$
		\item	The total flux of material through all pores is \[J = v\, \rho_f\, \epsilon = \dfrac{(D^2)(\Delta P)}{32 \mu \ell_M}\, \rho_f\, \epsilon = \dfrac{\Delta P}{\mu {\color{myOrange}\left(\dfrac{32 \,\ell_M}{\rho_f D^2 \,\epsilon } \right)}} = \dfrac{\Delta P}{\mu \, {\color{myOrange}R'_m}}\] % Seider, p 509, 510
