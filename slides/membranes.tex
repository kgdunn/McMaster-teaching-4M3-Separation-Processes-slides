	...
	\begin{columns}[c]
		\column{0.65\textwidth}
			...
			\begin{itemize}
				...
				\item	driving force = $\Delta P$:  0.1 to 1.0 MPa % Perry and C+Rv2
...
\begin{frame}\frametitle{Ultrafiltration transport phenomena}
	...
	\begin{itemize}		
		...
		\item	open, high-permeability side is primarily for mechanical strength % Perrys
		\item	precipitate forms colloidal gel on the membrane
		\item	reduced flux of permeate 
		\item	increasing $\Delta P$ leads to compacting this layer
		\item	so diminishing returns from increasing $\Delta P$
		\item	solute (i.e. particles) carried towards membrane with the solvent
...
\begin{frame}\frametitle{Transport through a UF membrane}
	Several transport phenomena happening:
	\begin{enumerate}
		\item	Movement of solute from the bulk, through polarization layer
	\end{enumerate}
\end{frame}

\begin{frame}\frametitle{Flux behaviour}
	Dilute solutions up to 1\% and 5 to 10psi 
	Linear region
	Perry: Fig20-63
...
\begin{frame}\frametitle{Terminology}
	\begin{itemize}
		\item	MWCO: measure of selectivity
		\begin{itemize}
			\item	e.g. MWCO of 10 kDa implies 90\% of feed at 10 kDa will be retained
			\item	subjective: does not specify TMP, feed concentration, \emph{etc}
		\end{itemize}
		\item	kDa
		\item	sieving curve: Perry 20-65
		\item	R =  1 − (Cp/Cf )
	\end{itemize}	\todo{Draw nonlinear flux vs pressure curve} 
...
\begin{comment}
	...
	\begin{frame}\frametitle{Geometries for ultrafiltration}
		% Spiral wound (Green book)
		% hollow fibres
		% 100 to 200 \micron, with walls of 25\micron: high surface area in a small volume; fibres can be spun at low cost

		% Perry: 
		 UF modules include cassettes, spirals, hollow fibers, tubes, flat sheets, and inorganic monoliths. These are primarily run in TFF operation to increase flux by reducing plugging

		Continuous operation provides lower-cost operation, but the biologicals process requires batch processing to meet regulatory requirements
