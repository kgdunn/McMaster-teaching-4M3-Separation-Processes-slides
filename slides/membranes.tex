...
\begin{frame}\frametitle{Question}
	A membrane operating with pure feed of water produces a flux of 0.20 $\text{kg}.\text{s}^{-1}\text{.m}^{-2}$.
	
	\begin{enumerate}
		\item	What is the resistance due to the membrane? Specify the units.
		\item	If operated with a protein-water mixture at a 20 kPa pressure difference, a flux of 0.04 $\text{kg}.\text{s}^{-1}\text{.m}^{-2}$ is measured at steady state. What is the resistance due to cake build-up? Specify the units.
	\end{enumerate}
\end{frame}
