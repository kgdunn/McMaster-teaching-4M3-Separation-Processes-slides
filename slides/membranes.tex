...
\begin{frame}\frametitle{Typical values of osmotic press}
	...
	\begin{tabular}{cll}
		...
		$T$ 		&  	[K]														& temperature\\
		$C$ 		&  	[mol of ions per {\color{myRed}{m$^{3}$}}]								& generic concentration
	\end{tabular}
...
\begin{frame}\frametitle{Simplified RO modelling}
	\begin{itemize}
		...
		\item	So $\Delta P$ overcome osmotic pressure and membrane resistance
	\end{itemize}
	\vspace{-6pt}
	\begin{enumerate}
		\item	\textbf{Solvent} flux
			...
			\[	R_{m,v} = f(\text{membrane's thickness, diffusivity of solvent in membrane})\]
		\item	\textbf{Solute} (\emph{salt}) flux
			...
			\]
			\begin{itemize}
				\item	Our assumption: $C_\text{wall} \approx C_\text{bulk}$ = bulk solute conc$^\text{n} \approx C_\text{feed}$ 
				\begin{itemize}
					\item	how would you enforce this assumption?
				\end{itemize}
				\item	$P_\text{solv}$ = permeability of the solvent 
				\item	$C_p$ = concentration of solute in the permeate
				\item	$P_\text{salt}$ = permeability of the salt
				\item	$\ell_M$ = membrane thickness				
			\end{itemize}			
	\end{enumerate}
...
\begin{frame}\frametitle{Example to try at home}
	...
	\vspace{6pt}
	The permeance of water has been established as $1.1 \times 10^{-4} \text{~kg.s}^{-1}\text{.m}^{-2}\text{.atm}^{-1}$ [how would you get this number?] and $16 \times 10^{-8} \text{m}^{3}\text{.s}^{-1}\text{.m}^{-2}$ for salt is determined experimentally (we won't cover this in the course).
