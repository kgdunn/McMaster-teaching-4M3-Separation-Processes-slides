...
% 2014: CO2 removal from natural gas: membranes, monoethanolamine absorber (packed bed). Cannot expand packed bed, but membrane: add more modules on if capacity increases; turn them off if less capacity required.
% Smaller in scale; e.g. can be used on off-shore platforms.
% Wiebe and Wieger report: In order to achieve the highest purity with the most output, a membrane should ideally have both high selectivity and high permeability. Higher permeability decreases the amount of membrane area required to treat a feed stream and high selectivity results in higher purity product gas [13]. Unfortunately as selectivity increases, permeability decreases and vice versa; this is one of the tradeoffs when using membrane technology [14]
%   [13] B. D. Freeman, “Basis of permeability/selectivity tradeoff relations in polymeric gas separation membranes,” Macromolecules, Vol. 32, No. 2 (1999), pp. 375-380. 
%   [14] M. Freemantle, “Membranes For Gas Separation ‐ Advanced organic and inorganic materials being developed for separations offer cost benefits for environmental and energy‐related processes,” Chemical & Engineering News, Vol. 83, No. 40 (2005),  pp. 49‐57.

% Cut out equations for J_salt = J_solv * CP : students are not using this correctly; cut out last few membrane slides and focus on other applications of membranes.
% Or check how RO designs are properly done.
...
\begin{frame}\frametitle{Example question}
	...
	%Due to gelling and fouling the flux cannot exceed 0.05 $\text{m}^3.\text{hour}^{-1}.\text{m}^{-2}$.
	% THis is wrong information. The upper limit is because when plotting Jv against log(Cb) you get an intercept (when Cb=0); this is the max flux possible with a clean membrane surface
...
\begin{frame}\frametitle{Other osmotic values}
	...
	\begin{itemize}
		\item	Molar mass of salt is $58.44\,\text{g.mol}^{−1}$
...
\begin{frame}\frametitle{Let's be a little more accurate}
	...
	\end{exampleblock}
	% We can do this by trying to use a higher Delta P. But then the retentate gets very high in concentration, 
	% creating an even greater osmotic pressure. So we have to use a higher Delta P to overcome this osmosis.
\end{frame}
...
\begin{frame}\frametitle{Industrial applications of RO}
	\begin{itemize}
		...
		\item	ultrahigh-purity water
		 ...
		 \item	other: keep antifreeze, paint, dyes, PAH, pesticides in retentate; discharge permeate to municipal wastewater % PAH = Polycyclic aromatic hydrocarbons
	\end{itemize}
\end{frame}
...
\begin{frame}\frametitle{Example to try}
	Brackish water of 1.8wt\% NaCl at 25$^{\circ}\text{C}$ and 1000 psia [68.5 atm] is fed to a spiral wound membrane.

	\vspace{6pt}
	Conditions on the permeate side are 0.05 wt\% NaCl, at the same temperature, but 50 psia [3.42 atm]
	...
	\begin{enumerate}
		...
		\item	How do these fluxes compare to each other? How do they compare to typical values?
		\item	Calculate the rejection coefficient, $R$, for salt.
	\end{enumerate}
	% pi_feed = c_feed x R x T
	% c_feed = 2 x 1.8 g NaCl/ 98.2 g H2O x 1000 g H2O/L x 1000 L/m^3 x 1 mol/58.4 g NaCl = 628 mol/m^3
	% pi_feed = 628 x 8.2057E-5 x 298 = 15.4 atm (reasonable, when compared to ocean water)	
	% pi_perm = 0.42 atm (similar calculation)
	
	% 1. Water flux = J = Asolv x (Delta P - Delta pi) with units of kg/(s.m^2)
	%    Volumetric flux = Jv = J / rho_f = 1.1E-4/1000 x ( (68.5 - 3.42) - (15.4 - 0.42)) = 5.51E-6 m^3/(s.m^2) x 3600 s/hr x 1000 L/m^3 = 19.8 LMH (compares well with RO)
	% 2. Salt flux = Jsalt = Asalt (Cw - Cp)
	%    Cw \approx Cfeed = 1.8 g salt/98.2 g water x 1 kg salt/1000 g salt x 1000 g water/1L x 1000 L/m^3 = 18.3 kg/m^3
	%    Cperm - 0.5 kg/m^3 in a similar calculation
	%    Salt flux  = 16E06 x (18.3 - 0.5) = 2.85E-6 kg salt/(s.m^2) x 3600 s/hr x 1000 g/kg = 10.2 g/(hr.m^2)
	% 3. Low salt flux, high water flux. Looks like good rejection
	% 4. Rejection coefficient = R = 1 - Cp / Cf = 1 - 0.5 / 18.3 = 97.3%. It is also R = 1 - pi_perm/pi_feed = 1 - c_perm / c_feed <--- units of mol/m^3 or units of pressure
...
\begin{frame}\frametitle{Some old and new terminology}
	...
	\begin{itemize}
		\item	$R = 1 - \dfrac{C_P}{C_\text{feed}} = 1 - \dfrac{C_P}{C_\text{0}}$ %and $R \approx 1 -  \dfrac{C_P}{C_\text{bulk}}$ if high inlet feed flows
...
\begin{comment}
	...
	% 2014: Put these 4 slides in the comments. Figure out how to teach this part better.
	%       Can you look up membranes is Seader, or Wankat book? T
	%		I did see another reference recently that did a better job
