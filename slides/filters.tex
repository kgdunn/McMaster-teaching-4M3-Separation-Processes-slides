...
% For 2015: change velocity "v" to "u" instead; avoids visual written conflict with filtrate volume "V"

% For 2015: reinclude this slide again (removed because I wanted to keep slides numbers aligned)
	...
	\begin{columns}[t]
		\column{0.60\textwidth}
			\begin{enumerate}
				...
				\vspace{36pt}
				\item	What factors can be used to {\color{myOrange}{\textbf{adjust}}} the unit's performance?
			\end{enumerate}
...
\begin{frame}\frametitle{Basic filtration equation recap}
	...
	\[
		\begin{array}{rcll}
			...
			-\Delta P_\text{tot}	&=& \text{total pressure drop} = -(\Delta P_c + P_m)	&[\text{Pa}]\\
			\mu 	&=& \text{fluid viscosity}  					&[\text{Pa.s}]\\
			C_S  	&=& \text{slurry concentration}					&[{\tiny \text{(kg dry solids)/(m}^{\tiny{3}}\text{ filtrate)}}]\\			
			R_m  &=& \text{resistance due to the medium}			&[\text{m}^{-1}]\\
			R_c  &=& \text{resistance due to the cake}				&[\text{m}^{-1}]\\
			\alpha	&=& \text{specific cake resistance} 			&[\text{m.kg}^{-1}]\\
...
\begin{frame}\frametitle{Example continued}
	...
	\vspace{12pt}
	\begin{enumerate}
		\item	Calculate the area required [i.e. how many plates are required in the frame?].
