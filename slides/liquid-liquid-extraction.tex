...
\begin{frame}\frametitle{Two \emph{counter-current} units}
	...
	\end{center}
	Note: each \textbf{\emph{difference}} is equal to $P$ (look on the flow sheet where those \emph{differences} are).
\end{frame}
...
\begin{frame}\frametitle{Counter-current graphical solution: 2 units}
	...
	\vspace{6pt}
	\begin{itemize}
		\item	$E_1$ is on the line that connects $F$ and $P$
		\item	$E_2$ is on the line that connects $R_1$ and $P$
		\item	$S$ is on the line that connects $R_2$ and $P$
	...
	\begin{columns}[t]
		\column{1.15\textwidth}
			\begin{enumerate}		
				...
				\begin{itemize}
					\item	1st cross-current stage, using a pure solvent flow of 50 kg/hr.
					\item	2nd cross-current stage, with an additional solvent flow of 50 kg/hr.
				\end{itemize}
				\item	For the overall 2-stage cross-current system, find the:
				\begin{itemize}
					\item	overall recovery {\small \color{myOrange}[\emph{answer:} $\sim$93\%]}
					\item	overall concentration of combined extract streams {\small \color{myOrange}[\emph{answer:} $\sim$21\%]}
				\end{itemize}				
