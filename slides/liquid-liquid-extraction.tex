	...
	\begin{columns}[t]
		\column{0.50\textwidth}
			\begin{itemize}
				...
				\item	In general: $y_{E1} > y_{E2} > \ldots$
...
\begin{frame}\frametitle{Counter-current graphical solution: 2 units}
	...
	\end{columns}
	\vspace{12pt}
	For example, let's require $x_{R2,A} = 0.05$ {\scriptsize (solute concentration in raffinate)}. What is $x_{E1,A}$ then {\scriptsize (concentration of solute in the extract)}?	
	\vfill
\end{frame}
	...
	\begin{columns}[t]
		...
		\column{0.40\textwidth}
			\begin{center}
				\includegraphics[width=\textwidth]{\imagedir/separations/liquid-liquid-extraction/counter-current-2-units.png}
			\end{center}
			\textbf{Note}: the line connecting {\color{myBlue}{\textbf{$E_1$ to $R_2$ is not a tie line}}}. We use the lever rule and a mass balance $(F + S = E_1 + R_2)$ to solve for all flows and compositions of $F, S, E_1,$ and $R_2$.
	\end{columns}
	\vspace{12pt}
...
\begin{frame}\frametitle{Counter-current graphical solution: minimal solvent flow}
	\begin{columns}[t]
		\column{0.70\textwidth}
			\begin{center}
	...
	{\small
		\begin{itemize}
			...
			\vspace{-6pt}
			\item	note that $R_2$ is always fixed (specified)
		\end{itemize}
	}
\end{frame}
...
\begin{frame}\frametitle{Counter-current graphical solution: 2 units}
	...
	\end{columns}
	\vspace{12pt}
	The last unit in a cascade is a special case: we have $R_{N=2}$, but we could have also calculated it from the tie line. Usually we have some overshoot or undershoot. {\scriptsize Good agreement in this example.}
\end{frame}
...
\begin{frame}\frametitle{In general: \emph{Counter-current} units}
	...
	\end{center}
		...
		\begin{center}		
			...
			\end{columns}
			\vspace{12pt}
			$F - E_1  =  R_1 - E_2 = \ldots = R_{n-1} - E_n = R_{n} - E_{n+1} = \ldots R_N - S = P$
		\end{center}
		Notes:
		\begin{enumerate}
